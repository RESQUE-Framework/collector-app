...
- `packs/dev/EP-theory_development.json` is the current theory-development expansion pack (version `5.0.0`, prefix `TH`). It implements the Theory Transparency Checklist v5.0 as 31 elements: four filter radio controls, one not-applicable explanation text field, one info field, and 25 checklist checkboxes (`T1`–`T5`, `F1`–`F5`, `S1`–`S5`, `E1`–`E5`, `C1`–`C5`).
